...
import com.example.assessment8.db.AppDB;
...
public class BACFragment extends Fragment {
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        if(item.getItemId() == R.id.action_add){
            ...
            db.drinksDao().deleteAll((ArrayList<Drink>) db.drinksDao().getAll());
    ...
    private void getDrinksAndNotifyAdapter(){
        ...
        db = Room.databaseBuilder(getActivity(), AppDB.class, "drinks-db")
                .fallbackToDestructiveMigration().allowMainThreadQueries()
                .build();

        mDrinks = (ArrayList<Drink>) db.drinksDao().getAll();
...
//        mDrinks.clear();
//        mDrinks.addAll(db.drinksDao().getAll());
//        adapter.notifyDataSetChanged();
//    }
